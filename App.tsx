...
const App: React.FC = () => {
  ...
  const handleRemoteEvent = (event: SyncEvent) => {
    switch (event.type) {
      ...
      case 'BID':
        const winner = INITIAL_FOUNDERS.find(f => f.id === event.winnerId) || null;
  ...
  // --- Timer Logic (Fixed: Removed internal recursion to prevent double-drops) ---
  useEffect(() => {
      ...
      timerRef.current = window.setTimeout(() => {
        setGameState(prev => {
          ...
          return {
            ...
            // Schedule next drop relative to now to maintain cadence
            nextDropTime: Date.now() + DROP_INTERVAL_MS,
            history: [...prev.history, { price: newPrice, timestamp: new Date(), event: 'DROP' }]
          };
        });
        
        // No recursive scheduleNextDrop() call here! 
        // The effect will re-run automatically because gameState.nextDropTime is a dependency.
      }, delay);
    } else {
      if (timerRef.current) {
  ...
  const handleBid = async (founder: Founder) => {
    ...
    // Only allow clicking your own button in remote mode
    ...
    if (isRemote) {
      ...
      soundService.playBid();
      // Brief aesthetic delay to capture the excitement of the click
    ...
    <div className="h-screen bg-slate-950 text-slate-100 flex flex-col overflow-hidden select-none">
      <header className="flex-none p-3 md:p-4 border-b border-slate-800 bg-slate-900/50 backdrop-blur-md z-50">
        <div className="w-full flex justify-between items-center px-1 md:px-4">
          <div className="flex items-center gap-2 md:gap-3">
            <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${isConnected ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-cyan-500'} animate-pulse-fast`}></div>
            <h1 className="text-xs md:text-xl font-bold tracking-tight text-white uppercase">DUTCH AUCTION {isRemote && <span className="text-cyan-400 font-mono ml-1 md:ml-2">REMOTE</span>}</h1>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
             {!isRemote ? (
               <button 
                 ...
                 className="text-[9px] md:text-[10px] uppercase font-bold text-slate-500 hover:text-cyan-400 transition-colors"
               >
                 Go Remote
               </button>
             ) : (
               <div className="flex items-center gap-2">
                 {!isConnected ? (
                   <div className="flex gap-1">
                      ...
                      />
                      <button onClick={joinRoom} className="bg-cyan-600 px-2 py-1 rounded text-[10px] font-bold">JOIN</button>
                   </div>
                 ) : (
                   <button onClick={toggleRemote} className="text-[9px] md:text-[10px] text-red-500 uppercase font-bold">Disconnect</button>
                 )}
               </div>
             )}
          ...
          <div className="bg-slate-900 border border-slate-800 p-8 md:p-12 rounded-3xl max-w-lg w-full text-center shadow-[0_0_50px_rgba(0,0,0,0.6)] border-t-slate-700/50">
            ...
            <div className="my-8 space-y-2">
              ...
              {gameState.winner ? (
                ...
                <div className="text-3xl md:text-5xl font-black text-slate-300 tracking-tighter">
                  FLOOR REACHED
                </div>
              )}
              <div className="text-2xl md:text-4xl font-bold text-white/90 tabular-nums mt-4">
            ...
            <div className="pt-6 border-t border-slate-800 mt-8 flex flex-col gap-3">
              ...
              {isRemote && (
                <p className="text-[10px] text-slate-500 font-mono italic">This will reset the room for everyone.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="flex gap-1.5 md:gap-2 overflow-x-auto pb-1 mask-linear scrollbar-hide">
               {gameState.history.slice().reverse().map((log, idx) => (
                 <div key={idx} className="flex-shrink-0 px-2 py-1 rounded bg-slate-900 border border-slate-800 text-[9px] md:text-[10px] font-mono whitespace-nowrap">
                    <span className={log.event === 'WIN' ? 'text-green-400' : 'text-slate-400'}>
               ...
               {gameState.history.length === 0 && (
                 <span className="text-[9px] md:text-[10px] text-slate-700 italic">Standby...</span>
               )}
            </div>
          </div>
        ...
        <div className="flex-none w-full md:w-72 lg:w-80 border-t md:border-t-0 md:border-l border-slate-800 bg-slate-900/40 p-3 md:p-6 z-10">
          <div className="grid grid-cols-3 md:flex md:flex-col gap-2 md:gap-4 h-full">
