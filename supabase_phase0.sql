...
create policy "auction_rooms write" on public.auction_rooms for insert to authenticated with check (
  exists (select 1 from public.rooms r where r.room_id = room_id and r.host_user_id = auth.uid())
);
create policy "auction_rooms update" on public.auction_rooms for update to authenticated 
  using (exists (select 1 from public.rooms r where r.room_id = room_id and r.host_user_id = auth.uid()))
  with check (exists (select 1 from public.rooms r where r.room_id = room_id and r.host_user_id = auth.uid()));
...
create policy "auction_events insert" on public.auction_events for insert to authenticated with check (
  exists (select 1 from public.rooms r where r.room_id = room_id and r.host_user_id = auth.uid())
  or
  exists (select 1 from public.room_participants p where p.room_id = room_id and p.user_id = auth.uid())
);
